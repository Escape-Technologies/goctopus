...
func worker(domains chan string, output chan fingerprint.Output, workerId int, wg *sync.WaitGroup) {
	defer wg.Done()
	log.Debugf("Worker %d instantiated\n", workerId)
	for domain := range domains {
		log.Debugf("Worker %d started on: %v\n", workerId, domain)
		res, err := fingerprint.FingerprintDomain(domain)
		if err == nil {
...
func Orchestrator(inputBuffer *bufio.Scanner, maxWorkers int, output chan fingerprint.Output, count int) {
